...
Project default workflow depth: after decomposing, `/scope` also recommends a project wide default (`Vibe` / `Lean` / `Medium` / `Full`) as a decision panel with exactly one recommended, explaining the stages each level runs, and records it in the scope header `**Workflow:**` line (plan Step 5b). It is a recommendation the engineer overrides, never a cold question. The tier sets the stages after `/develop` AND what closes a feature to `done` (the last required stage marks it: `Vibe` → `/develop` on build plus self check, `Lean` → `/check verify`, `Medium`/`Full` → `/test`); an `Assumed` spec still blocks `done` at every tier. It sets the baseline each feature inherits; a feature's own higher Weight tag still wins. `/develop` reads it to scale the next steps it recommends after a build, so a `Vibe` project is not told to run verify, and a lean project is not told to run a full review chain.
